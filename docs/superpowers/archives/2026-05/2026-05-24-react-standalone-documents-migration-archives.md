...
- 将 `scripts/dev-start.ps1` / `scripts/dev-stop.ps1` 对齐到当前双进程开发模式：支持一键启动、`-Target Server` / `-Target React` 单侧启动、已运行服务复用，并新增 Git Bash 可用的 `.sh` 包装脚本。
...
- Dev startup verification: `scripts/dev-start.ps1 -SkipNpmInstall -SkipClientBuild` started Server and React; `http://127.0.0.1:5173/documents` returned `200`; `http://localhost:5261/api/MarkdownDocuments` returned `200`; `-Target Server`, `-Target React`, `-Target Service`, process reuse, `dev-stop`, and Git Bash wrappers were exercised successfully.
