...
RAGAS evaluation run store 也可能出现同族症状，文件名通常是唯一临时文件而不是目标 JSON：
...
RAGAS env-key coordinator test 的直接根因同样不是目录权限，而是测试提前清理：`CreateAsync` 返回 `Queued` 后，后台 runner 仍可能在写 run store 临时文件；测试类 `Dispose` 立即递归删除临时 WorkingDir，就会撞上 Windows 文件句柄占用。
...
- 堆栈停在测试类 `Dispose` / `Directory.Delete(..., recursive: true)`，目标文件是 `ragas_runs.json.<guid>.tmp`，且测试刚通过 coordinator 创建过后台 run。
